...
//TABLES FROM GCC STD LIB
static const unsigned short char_from [] =
{
...
static const unsigned short char_to [] =
{
...
static void ushort_to_utf8(unsigned short achar, char* out)
{
    if (achar < 0x007F)
    {
        (*out)=achar;
    ...
    {
        out[0]  = 0xC0 | ( ( achar >> 6 ) & 0x1F ); // 110 xxxxx
        out[1]  = 0x80 | ( achar & 0x3F );          // 10  xxxxxx
    }
    else
    {
        out[0]  = 0xE0 | ( ( achar >> 12 ) & 0x000F );// 1110 xxxx
        out[1]  = 0x80 | ( ( achar >> 6  ) & 0x003F );// 10   xxxxxx
        out[2]  = 0x80 | ( achar & 0x3F );            // 10   xxxxxx
    }
}

static unsigned short utf8_to_lower_ushort(const char* achar)
{
    
    const unsigned short wc = utf8_to_ushort(achar);
    const unsigned short *l = char_from;
    const unsigned short *r = char_from + ARRAY_SIZE;
    const unsigned short *s = NULL;
...
static unsigned short utf8_to_upper_ushort(const char* achar)
{
    
    const unsigned short wc = utf8_to_ushort(achar);
    const unsigned short *l = char_to;
    const unsigned short *r = char_to + ARRAY_SIZE;
    const unsigned short *s = NULL;
    ...
    //not found
    if (l == char_to + ARRAY_SIZE)
        return wc;
    //found
    if (wc == *l)
        return char_from [l - char_to];
    //not
    else
        return wc;
}
                                
static void utf8_to_lower_char(const char* achar,char* out)
{
    ushort_to_utf8(utf8_to_lower_ushort(achar),out);
}
                                
static void utf8_to_upper_char(const char* achar,char* out)
{
    ushort_to_utf8(utf8_to_upper_ushort(achar),out);
}

inline static size_t utf8_next_char_count(char c)
{
...
static void utf8_next_char(char*& achar)
{
    achar+=utf8_next_char_count(*achar);
}

static void _utf8_to_lower_str(char*& achar)
{
    while(*achar)
    {
        utf8_to_lower_char(achar,achar);
        utf8_next_char(achar);
    }
}
                                
static void _utf8_to_upper_str(char*& achar)
{
    while(*achar)
    {
        utf8_to_upper_char(achar,achar);
        utf8_next_char(achar);
    }
}

void StringUtils::to_lower_utf8(std::string& outstr)
{
    char* ptr_to_data= (char*)outstr.data();
    _utf8_to_lower_str(ptr_to_data);
}

void StringUtils::to_upper_utf8(std::string& outstr)
{
    char* ptr_to_data=  (char*)outstr.data();
    _utf8_to_upper_str(ptr_to_data);
}